...
def mypage():
    if 'user_id' not in session: #로그인상태인지 확인
        return redirect(url_for('login')) #로그인아니면 htp://localhost:5000/login으로 보냄
    ...
    try:
        ...
        # 1. 내 상세 정보 조회
            ...
            # 결과를 리턴                                mypage.html에게 user 객체와 board_count객체를 담아 보냄
            # 프론트에서 사용할려면{{user.???}}{{board_count}}
    finally:
        conn.close()
...
@app.route('/board') #http://localhost:500/board
def board_list():
    conn = Session.get_connection()
    try:
        with conn.cursor() as cursor:
...
################################################# 성적 관리 CRUD #########################################################
# 주의사항: ROLE에 ADMIN과 MANANER만 CUD만 제공.
# 유저는 본인 성적만 조회가 가능함.
...
def download_file(filename):
    ...
    origin_name = request.args.get('origin_name')           #  as_attachment : 그림 눌리면 다운 받게 해주기
